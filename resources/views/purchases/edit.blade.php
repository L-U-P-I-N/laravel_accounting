...
@php
    ...
    $purchaseDateValue = old('purchase_date', optional($purchase->purchase_date)->format('Y-m-d'));
    $dueDateValue = old('due_date', optional($purchase->due_date)->format('Y-m-d'));
    $supplierIdValue = old('supplier_id', $purchase->supplier_id);
    $paymentStatusValue = old('payment_status', $purchase->payment_status);
    $paymentAccountIdValue = old('payment_account_id', $purchase->payment_account_id);
    $statusValue = old('status', $purchase->status);
    $notesValue = old('notes', $purchase->notes ?? '');
    $supplierInvoiceNumberValue = old('supplier_invoice_number', $purchase->supplier_invoice_number ?? '');
@endphp

@section('title', 'تعديل طلب الشراء ' . $purchase->purchase_number)
...
<div class="page-header">
    <div>
        <h2 class="page-title"><i class="fas fa-edit"></i> تعديل طلب الشراء {{ $purchase->purchase_number }}</h2>
        <p class="text-muted mt-2 mb-0">تحديث بيانات طلب الشراء الحالي</p>
    </div>
...
<div class="purchase-form">
    @php
        ...
        $purchaseItems = $hasErrors
            ? collect(old('item_description', []))->map(function ($description, $index) use ($defaultTaxRate) {
                return (object) [
    ...
    <form method="POST" action="{{ route('purchases.update', $purchase) }}" class="purchase-form-full" enctype="multipart/form-data" data-purchase-form data-paid-amount="{{ number_format((float) $purchase->paid_amount, 2, '.', '') }}">
        @csrf
        @method('PUT')
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-md-3">
                ...
                <input type="date" name="payment_date" class="form-control @error('payment_date') is-invalid @enderror"
                       value="{{ old('payment_date', optional($purchase->payment_date)->format('Y-m-d')) }}">
                @error('payment_date')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-6">
                ...
                @if ($purchase->attachment_path)
                    <div class="attachment-info">
                        ...
                        <a href="{{ route('purchases.attachment', $purchase) }}" target="_blank" class="text-decoration-none">
                            عرض الملف المرفق الحالي
                        </a>
                    </div>
                @endif
                @error('attachment')
                ...
                <tbody data-purchase-items>
                    @foreach ($purchaseItems as $item)
                        <tr data-purchase-item-row>
                            ...
                            <td>
                                <input type="number" name="item_cost_price[]" class="form-control purchase-item-cost" min="0" step="0.01" value="{{ $item->cost_price ?? 0 }}" lang="en" dir="ltr">
                            </td>
                            <td>
                            ...
                            <td>
                                <input type="number" name="item_tax_rate[]" class="form-control purchase-item-tax" min="0" max="100" step="0.01" value="{{ $item->tax_rate ?? 15 }}" lang="en" dir="ltr">
                            </td>
                            <td>
        ...
        <div class="row mt-4 g-3 total-section">
            ...
            <div class="col-md-2">
                <div class="text-center">
                    ...
                    <div class="h5 mb-0" data-purchase-paid-summary>{{ number_format((float) $purchase->paid_amount, 2) }} {{ $company->currency }}</div>
                </div>
            </div>
            <div class="col-md-2">
                <div class="text-center">
                    <strong>المتبقي:</strong>
                    <div class="h5 mb-0" data-purchase-remaining>{{ number_format((float) $purchase->balance_due, 2) }} {{ $company->currency }}</div>
                </div>
            </div>
            <div class="col-md-2">
                <button type="submit" class="btn btn-save w-100">
                    <i class="fas fa-save me-2"></i>تحديث الطلب
                </button>
            </div>
        </div>
    </form>
...
function calculatePurchaseTotals(form) {
    ...
    // Get paid amount from data attribute for edit form
    const paidAmount = parseFloat(form.dataset.paidAmount) || 0;
